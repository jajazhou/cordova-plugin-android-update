...
public class CheckUpdateThread implements Runnable {
  ...
  private Version getUpdateVersion() {
    int force = 1;// Peter, 20240412: 0 可选更新, 1 强制更新
    int localCode = 0;
    int remoteCode = 0;
    String remoteName = "";
    String remoteRemark = "";
    ...
    if (null != getMHashMap()) {
      force = Integer.valueOf(getMHashMap().get("force"));// Peter, 20240412: 0 可选更新, 1 强制更新
      remoteCode = Integer.valueOf(getMHashMap().get("version"));
      remoteName = getMHashMap().get("name");
      remoteRemark = getMHashMap().get("remark");
    }
    return new Version(force, localCode, remoteCode, remoteName, remoteRemark);
  }
}