...
import androidx.core.content.FileProvider;// Peter, 20240412
...
public class DownloadHandler extends Handler {
  ...
  public DownloadHandler(Context mContext, ProgressBar mProgress, AlertDialog mDownloadDialog, String mSavePath, HashMap<String, String> mHashMap, String mFileName) {
    this.msgHelper = new MsgHelper(mContext.getPackageName(), mContext.getResources());
    this.mDownloadDialog = mDownloadDialog;
    this.mContext = mContext;
    this.mProgress = mProgress;
    this.mSavePath = mSavePath;
    this.mFileName = mFileName;// Peter, 20240412:下载保存的临时文件名称
  ...
  private void installApk() {
    LOG.d(TAG, "Installing APK:"+this.mFileName);

    //File apkFile = new File(mSavePath, mHashMap.get("name") + ".apk");
    // Peter, 20240412: 改为唯一的临时apk文件名
    File apkFile = new File(mSavePath, this.mFileName);
    if (!apkFile.exists()) {
